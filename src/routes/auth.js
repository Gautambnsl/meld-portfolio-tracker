...
router.post('/signup', async (req, res) => {
  ...
  try {
    ...
    user = new User({
      ...
      phoneNumber, 
      tempAddress,
      premiumService: 'free', // Set to free by default
      isAlertOn: false, // Alerts off by default
      alertObjects: [] // Empty alert objects by default
    });
...
router.put('/user/update', verifyToken, async (req, res) => {
  const userId = req.userId;
  const { phoneNumber, tempAddress, walletAddress, isAlertOn, alertObjects } = req.body;
  ...
  try {
    ...
    if (isAlertOn !== undefined) user.isAlertOn = isAlertOn; // Update alert status
    if (alertObjects) user.alertObjects = alertObjects; // Update alert objects
...
router.put('/user/rootUpdate', verifyToken, async (req, res) => {
